...
function isChecklist11CandidateAllowed(article: string) {
  ...
  const procedureOnly = procedureOnlyTerms.some((word) => includesLoose(article, word));
  if (procedureOnly && !hasStructure && !hasQuorum) return false;
...
function scoreArticle(article: string, rule: Rule) {
  ...
  if (isChecklist11CommitteeStructureRule(rule)) {
    if (!isChecklist11CandidateAllowed(article)) score -= 120;
    if (hasCommitteeIdentity(article) && hasCommitteeStructureTerms(article)) score += 90;
    if (hasCommitteeIdentity(article) && hasCommitteeQuorumTerms(article)) score += 70;
    if (includesLoose(titleArea, "구성") || includesLoose(titleArea, "정족수")) score += 60;
  }
