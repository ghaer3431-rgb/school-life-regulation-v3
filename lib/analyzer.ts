// CHECKLIST3_SCOPE_FALLBACK_PATCH_FIXED
...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    checklistNo: "3",
    category: "생활지도",
    title: "생활지도의 범위 중 ‘그 밖의 분야’ 및 스마트기기 사용 지도 반영",
    basis: "고시 연계",
    groups: [["그 밖의 분야"], ["특수교육대상자", "특수교육"], ["다문화"], ["용모", "복장"], ["비행", "범죄"], ["휴대전화 등 스마트기기", "스마트기기", "휴대전화"], ["올바른 사용"]],
    titleKeywords: ["그 밖의 분야", "생활지도의 범위"],
    targetKeywords: ["생활지도의 범위", "그 밖의 분야", "특수교육대상자", "특수교육", "다문화", "용모", "복장", "비행", "범죄", "휴대전화 등 스마트기기", "스마트기기", "휴대전화", "올바른 사용"],
    sources: [src.training, src.notice],
    revised:
...
4. 휴대전화 등 스마트기기의 올바른 사용에 관한 사항`,
    reason:
      "상향입법으로 신설된 필수 반영 사항입니다. 생활지도의 범위 중 ‘그 밖의 분야’에 특수교육대상자와 다문화학생에 대한 인식 및 태도, 용모 및 복장, 비행 및 범죄 예방, 휴대전화 등 스마트기기의 올바른 사용에 관한 사항이 포함되어야 합니다.",
  },
  {
...
출석정지는 1회 최대 10일 이내, 연간 30일 이내로 하며, 출석정지 기간은 학교생활기록부의 출결상황란에 미인정 결석 일수에 산입하여 기재하되 특기사항란에 사유는 기재하지 않는다. 초등학교와 중학교는 퇴학처분을 둘 수 없다.`,
  ...
  },
  {
    ...
    checklistNo: "추가",
    category: "용어 정비",
    title: "‘교육목적상 필요한 지도 방법’ 장·절 제목 확인",
    basis: "연수자료 근거",
...
제○절 교육목적상 필요한 지도 방법`,
    reason:
      "초·중등교육법 시행령 제9조제1항제7호 및 연수자료의 표현에 맞추어 ‘교육목적상 필요한 지도 방법’ 장·절 제목이 반영되어 있는지 확인합니다. 모산초 생활규정 확정안과 강릉해람중 생활규정안 모두 ‘생활지도 및 교육목적상 필요한 지도 방법’ 형태의 장 제목을 사용하므로 이를 개정안 예시의 기준 표현으로 삼습니다. ‘징계 외 지도방법’은 오래된 규정에 남아 있을 수 있는 구식 표현으로만 점검하며, 일반적인 ‘징계’, ‘징계의 종류’, ‘징계 심의’ 조항 전체는 이 항목의 후보로 보지 않습니다.",
  },
];
...
function isChecklist3GuidanceScopeRule(rule: Rule) {
  return rule.id === "cl03-guidance-fields";
}

function hasGuidanceOtherFieldsTitle(article: string) {
  return includesLoose(article.slice(0, 260), "그 밖의 분야") || includesLoose(article.slice(0, 260), "생활지도의 범위");
}

function hasGuidanceOtherFieldsBody(article: string) {
  return includesLoose(article, "그 밖의 분야");
}

function hasGuidanceScopeAnchor(article: string) {
  const hasStudyCareer = includesLoose(article, "학업 및 진로");
  const hasHealthSafety = includesLoose(article, "보건 및 안전");
  const hasCharacterRelation = includesLoose(article, "인성 및 대인관계");
  const hasGuidanceScope = includesLoose(article, "생활지도의 범위");
  const hasEducationGuidanceTitle = includesLoose(article, "생활지도 및 교육목적상 필요한 지도 방법");

  return hasGuidanceScope || hasEducationGuidanceTitle || (hasStudyCareer && hasHealthSafety && hasCharacterRelation);
}

function hasGuidanceScopeAllItems(article: string) {
  const hasSpecial = includesLoose(article, "특수교육대상자") || includesLoose(article, "특수교육");
  const hasMulti = includesLoose(article, "다문화");
  const hasAppearance = includesLoose(article, "용모") || includesLoose(article, "복장");
  const hasDelinquency = includesLoose(article, "비행") || includesLoose(article, "범죄");
  const hasSmart = (includesLoose(article, "스마트기기") || includesLoose(article, "휴대전화")) && includesLoose(article, "올바른 사용");

  return hasSpecial && hasMulti && hasAppearance && hasDelinquency && hasSmart;
}

function isChecklist3CandidateAllowed(article: string) {
  // 3번은 정보통신/스마트기기 조항 전체가 아니라 '생활지도의 범위' 묶음이나 '그 밖의 분야' 조항을 찾아야 한다.
  if (hasGuidanceOtherFieldsBody(article)) return true;
  if (hasGuidanceScopeAllItems(article)) return true;
  if (hasGuidanceScopeAnchor(article)) return true;
  return false;
}

function aroundIndex(source: string, index: number, before = 900, after = 1800) {
  return source.slice(Math.max(0, index - before), Math.min(source.length, index + after));
}

function findNearestArticleStart(source: string, index: number) {
  const articlePattern = /제\s*\d+\s*조[^\n]{0,40}/g;
  let last = -1;
  let match: RegExpExecArray | null;

  while ((match = articlePattern.exec(source)) !== null) {
    if (match.index <= index) last = match.index;
    if (match.index > index) break;
  }

  return last >= 0 ? last : Math.max(0, index - 800);
}

function findNextArticleStart(source: string, index: number) {
  const rest = source.slice(index + 10);
  const match = rest.match(/제\s*\d+\s*조[^\n]{0,40}/);
  return match && typeof match.index === "number" ? index + 10 + match.index : -1;
}

function findChecklist3FallbackCandidate(fullText: string): ArticleCandidate[] {
  const normalizedText = normalize(fullText);
  const candidates: ArticleCandidate[] = [];

  const keywordGroups: Array<[string, number]> = [
    ["그 밖의 분야", 120],
    ["생활지도의 범위", 105],
    ["생활지도 및 교육목적상 필요한 지도 방법", 100],
    ["학업 및 진로", 80],
    ["보건 및 안전", 75],
    ["인성 및 대인관계", 75],
    ["스마트기기의 올바른 사용", 70],
    ["휴대전화 등 스마트기기의 올바른 사용", 70],
  ];

  for (const [keyword, score] of keywordGroups) {
    const index = normalizedText.indexOf(keyword);
    if (index < 0) continue;

    let start = findNearestArticleStart(normalizedText, index);
    let end = findNextArticleStart(normalizedText, index);

    // 생활지도의 범위는 여러 조항이 연속될 수 있으므로 넓게 보여준다.
    if (["생활지도의 범위", "생활지도 및 교육목적상 필요한 지도 방법", "학업 및 진로", "보건 및 안전", "인성 및 대인관계"].includes(keyword)) {
      const otherIndex = normalizedText.indexOf("그 밖의 분야", index);
      if (otherIndex > index) {
        const nextAfterOther = findNextArticleStart(normalizedText, otherIndex);
        end = nextAfterOther > 0 ? nextAfterOther : Math.min(normalizedText.length, otherIndex + 1500);
      } else {
        end = Math.min(normalizedText.length, index + 3500);
      }
      start = Math.max(0, start);
    }

    if (end < 0 || end <= start) end = Math.min(normalizedText.length, index + 1800);

    const text = normalizedText.slice(start, end);
    if (text.length < 40) continue;

    if (!candidates.some((candidate) => candidate.text === text || Math.abs(normalizedText.indexOf(candidate.text.slice(0, 30)) - start) < 80)) {
...
function scoreArticle(article: string, rule: Rule) {
  ...
  if (isChecklist3GuidanceScopeRule(rule)) {
    if (!isChecklist3CandidateAllowed(article)) score -= 220;
    if (hasGuidanceOtherFieldsTitle(article)) score += 150;
    if (hasGuidanceOtherFieldsBody(article)) score += 110;
    if (hasGuidanceScopeAnchor(article)) score += 90;
    if (hasGuidanceScopeAllItems(article)) score += 150;
    if ((includesLoose(article, "스마트기기") || includesLoose(article, "휴대전화")) && includesLoose(article, "올바른 사용")) score += 70;
  }
...
function findArticleCandidates(text: string, rule: Rule): ArticleCandidate[] {
  ...
  const ranked = articles
    ...
    .filter((item) => rule.id !== "cl03-guidance-fields" || isChecklist3CandidateAllowed(item.text))
  ...
  if (rule.id === "cl03-guidance-fields") {
    return findChecklist3FallbackCandidate(text);
  }
...
function getStatus(text: string, rule: Rule, schoolLevel: SchoolLevel, confidence: string): ReviewStatus {
  ...
  if (isChecklist3GuidanceScopeRule(rule)) {
    const hasAllItems = hasGuidanceScopeAllItems(text);

    // 상향입법으로 신설된 필수 반영 사항이므로,
    // '생활지도의 범위 - 그 밖의 분야' 안에 스마트기기의 올바른 사용이 포함되어야만 적정으로 본다.
    if (hasAllItems) return confidence === "낮음" ? "확인 필요" : "적정";
    return "보완 필요";
  }
...
function getComment(status: ReviewStatus, confidence: string, ruleId?: string) {
  if (ruleId === "cl03-guidance-fields") {
    if (status === "적정") return "생활지도의 범위 중 ‘그 밖의 분야’에 ‘휴대전화 등 스마트기기의 올바른 사용에 관한 사항’이 포함되어 있는 것으로 확인됩니다.";
    return "상향입법으로 신설된 필수 반영 사항입니다. 아래에 추출된 ‘생활지도의 범위’ 또는 ‘그 밖의 분야’ 조항을 확인하고, ‘휴대전화 등 스마트기기의 올바른 사용에 관한 사항’이 없으면 보완이 필요합니다.";
  }
  if (ruleId === "cl16-guidance-method-title") {
    if (status === "적정") return "‘교육목적상 필요한 지도 방법’ 표현이 확인됩니다.";
    if (status === "보완 필요") return "구식 표현인 ‘징계 외 지도방법’이 확인됩니다. 장·절 제목을 ‘교육목적상 필요한 지도 방법’으로 정비하는 것을 검토하세요.";
    return "‘교육목적상 필요한 지도 방법’ 장·절 제목을 자동 확인하지 못했거나 구식 표현과 신규 표현이 함께 확인됩니다. 담당자가 장·절 제목을 확인하세요.";
  }
  if (status === "적정") {
