...
const RULES: Rule[] = [
  {
    ...
    basis: "체크리스트 필수",
    groups: [["초·중등교육법", "초중등교육법"], ["교원의 학생생활지도에 관한 고시", "학생생활지도에 관한 고시"]],
    targetKeywords: ["적용근거", "근거", "초·중등교육법", "교원의 학생생활지도에 관한 고시", "2026"],
  ...
  },
  {
    ...
    targetKeywords: ["생활지도", "그 밖의 분야", "특수교육", "다문화", "용모", "복장", "비행", "범죄", "스마트기기"],
  ...
  },
  {
    ...
    basis: "법령 연계",
    groups: [["스마트기기"]],
    targetKeywords: ["휴대전화", "휴대폰", "통신기기", "스마트기기"],
  ...
  },
  {
    ...
    targetKeywords: ["주의", "스마트기기", "물품", "장애", "특수교육", "교육 목적", "긴급"],
    sources: [src.checklist, src.law, src.notice, src.training],
    revised:
  ...
  },
  {
    ...
    targetKeywords: ["훈육", "스마트기기", "사용", "소지", "제한", "기준", "방법", "유형"],
    sources: [src.checklist, src.law, src.notice, src.training],
    revised:
  ...
  },
  {
    ...
    targetKeywords: ["훈육", "분리", "보관", "물품", "스마트기기", "소지", "사용", "반환"],
    sources: [src.checklist, src.notice, src.training],
    revised:
...
function groupMatched(text: string, group: string[]) {
  ...
  return group.some((keyword) => keyword && text.includes(keyword));
}
...
function hasSchoolLevelProblem(text: string, rule: Rule, schoolLevel: SchoolLevel) {
  if (rule.id === "cl13-five-disciplinary-measures") {
    if ((schoolLevel === "초등학교" || schoolLevel === "중학교") && text.includes("퇴학")) return true;
    if (schoolLevel === "고등학교" && !text.includes("퇴학")) return true;
  }
  return false;
}
...
function scoreArticle(article: string, rule: Rule) {
  ...
  for (const keyword of rule.targetKeywords) {
    if (keyword && article.includes(keyword)) score += 6;
  }

  for (const keyword of rule.outdated ?? []) {
    if (keyword && article.includes(keyword)) score += 3;
  }

  for (const keyword of rule.forbidden ?? []) {
    if (keyword && article.includes(keyword)) score += 10;
  }

  if (article.includes("제") && article.includes("조")) score += 3;
  if (/[①②③④⑤⑥⑦⑧⑨⑩]/.test(article)) score += 2;
  if (article.length > 80) score += 2;
  if (article.length > 500) score -= 2;
  if (article.includes("그림입니다") || article.includes("원본 그림")) score -= 10;
  if (article.includes("목차") && article.length < 250) score -= 12;
...
function findArticleCandidates(text: string, rule: Rule): ArticleCandidate[] {
  ...
  const prepared = normalizeForArticleSearch(text);
  const keys = [...rule.targetKeywords, ...rule.groups.flat(), ...(rule.outdated ?? []), ...(rule.forbidden ?? [])].filter(Boolean);
  ...
  for (const keyword of keys) {
    ...
    const start = before >= 0 ? before + 1 : Math.max(0, index - 350);
    const end = after > start ? after : Math.min(prepared.length, index + 900);
...
function renderCandidates(candidates: ArticleCandidate[], confidence: string) {
  if (candidates.length === 0) {
    return `자동 추출 신뢰도: 낮음\n관련 조항을 자동으로 찾지 못했습니다. 문서 전체에서 담당자 확인이 필요합니다.`;
  }

  const body = candidates
    .map((candidate, index) => `[후보 ${index + 1} / 점수 ${candidate.score}]\n${candidate.text.slice(0, 1000)}`)
    .join("\n\n");

  return `자동 추출 신뢰도: ${confidence}\n※ 현행 조항은 관련 후보를 최대 3개까지 표시합니다. PDF 추출 오류가 있을 수 있으므로 후보 조항을 확인하세요.\n\n${body}`;
}
...
export function analyzeRegulation(params: {
  ...
  const items: ReviewItem[] = activeRules.map((rule) => {
    ...
    return {
      ...
      current: text.length < 50 ? "규정 본문이 충분히 입력되지 않았습니다." : renderCandidates(candidates, confidence),
