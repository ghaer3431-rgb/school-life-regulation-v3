// CHECKLIST3_STRICT_REQUIRED_PATCH_FIXED
...
function isChecklist3CandidateAllowed(article: string) {
  // 3번은 정보통신/스마트기기 조항 전체가 아니라 '생활지도의 범위 - 그 밖의 분야' 조항을 찾아야 한다.
...
function findChecklist3FallbackCandidate(fullText: string): ArticleCandidate[] {
  ...
  const keywords = ["그 밖의 분야", "휴대전화 등 스마트기기", "스마트기기의 올바른 사용", "올바른 사용에 관한 사항"];
  let bestIndex = -1;
  ...
  const articleStart = Math.max(
    0,
    normalizedText.lastIndexOf("제", bestIndex) >= 0 ? normalizedText.lastIndexOf("제", bestIndex) : bestIndex - 700
  );
  const nextArticle = normalizedText.indexOf("제", bestIndex + 20);
  const articleEnd = nextArticle > bestIndex ? nextArticle : Math.min(normalizedText.length, bestIndex + 1500);
  const candidateText = normalizedText.slice(articleStart, articleEnd);

  if (!isChecklist3CandidateAllowed(candidateText)) {
    return [{
      text: normalizedText.slice(Math.max(0, bestIndex - 700), Math.min(normalizedText.length, bestIndex + 1200)),
      score: 40,
    }];
  }

  return [{ text: candidateText, score: 95 }];
}
...
function scoreArticle(article: string, rule: Rule) {
  ...
  if (isChecklist3GuidanceScopeRule(rule)) {
    ...
    if (hasGuidanceOtherFieldsTitle(article)) score += 140;
    if (hasGuidanceOtherFieldsBody(article)) score += 90;
    if (hasGuidanceScopeAllItems(article)) score += 120;
...
function getComment(status: ReviewStatus, confidence: string, ruleId?: string) {
  if (ruleId === "cl03-guidance-fields") {
    ...
    return "상향입법으로 신설된 필수 반영 사항입니다. 생활지도의 범위 중 ‘그 밖의 분야’ 조항에 ‘휴대전화 등 스마트기기의 올바른 사용에 관한 사항’이 없으면 보완이 필요합니다.";
  }
