...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    basis: "교육청 권고",
    groups: [["학생선도위원회", "선도위원회"], ["구성", "부위원장", "교감", "부장", "위원으로 구성"], ["과반수", "정족수", "찬성", "재적"]],
    titleKeywords: ["학생선도위원회", "선도위원회", "구성", "정족수"],
    targetKeywords: ["학생선도위원회", "선도위원회", "구성", "부위원장", "교감", "부장", "과반수", "정족수", "찬성", "재적"],
...
function hasCommitteeCompositionTerms(article: string) {
  // '위원장에게 보고' 같은 절차 표현은 구성으로 보지 않는다.
  const strongCompositionTerms = [
    "구성",
    "위원으로 구성",
    "위원은",
    "위원 수",
  ...
  return strongCompositionTerms.some((word) => includesLoose(article, word));
}
...
function hasCommitteeQuorumTerms(article: string) {
  // 11번의 정족수는 단순한 '출석'이나 '의결'이 아니라 과반수/정족수/출석+찬성 표현이어야 한다.
...
function isChecklist11CandidateAllowed(article: string) {
  ...
  // 절차 소음이 있고, 강한 구성/정족수 표현이 없으면 제외한다.
  if (hasNoise && !hasComposition && !hasQuorum) return false;

  // 11번은 반드시 구성 또는 정족수 표현이 있어야 한다.
  return hasComposition || hasQuorum;
}
...
function scoreArticle(article: string, rule: Rule) {
  ...
  if (isChecklist11CommitteeStructureRule(rule)) {
    ...
    if (hasCommitteeIdentity(article) && hasCommitteeCompositionTerms(article)) score += 120;
    if (hasCommitteeIdentity(article) && hasCommitteeQuorumTerms(article)) score += 100;
    if (includesLoose(titleArea, "구성") || includesLoose(titleArea, "정족수")) score += 80;
    if (hasChecklist11ProcedureNoise(article) && !hasCommitteeCompositionTerms(article) && !hasCommitteeQuorumTerms(article)) score -= 200;
  }
