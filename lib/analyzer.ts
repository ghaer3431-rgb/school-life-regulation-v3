// CL11_QUORUM_STRICT_PATCH_FIXED
...
function hasCommitteeStructureTerms(article: string) {
  return ["구성", "위원장", "부위원장", "교감", "부장", "위원으로 구성", "위원은", "위원 수", "교원 5인", "5인 이상", "10인 이하"].some((word) => includesLoose(article, word));
}
...
function isChecklist11CandidateAllowed(article: string) {
  ...
  if (!hasCommitteeIdentity(article)) return false;
  if (excludedCommitteeTerms.some((word) => includesLoose(article, word))) return false;
  if (hasOnlyCommitteeProcedureTerms(article)) return false;

  return hasCommitteeStructureTerms(article) || hasCommitteeQuorumTerms(article);
}
...
function scoreArticle(article: string, rule: Rule) {
  ...
  if (isChecklist11CommitteeStructureRule(rule)) {
    if (!hasCommitteeIdentity(article)) score -= 60;
    if (hasCommitteeIdentity(article) && hasCommitteeStructureTerms(article)) score += 80;
    if (hasCommitteeIdentity(article) && hasCommitteeQuorumTerms(article)) score += 60;
    if (includesLoose(titleArea, "구성") || includesLoose(titleArea, "정족수")) score += 55;
    if (hasOnlyCommitteeProcedureTerms(article)) score -= 90;
    if (!isChecklist11CandidateAllowed(article)) score -= 80;
  }
