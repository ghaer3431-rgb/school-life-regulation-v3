...
function getFocusKeywords(rule: Rule) {
  return [
    ...rule.targetKeywords,
    ...rule.groups.flat(),
    ...(rule.titleKeywords ?? []),
...
function focusedExcerpt(article: string, rule: Rule) {
  const keywords = getFocusKeywords(rule);
  ...
  for (const keyword of keywords) {
    const index = normalizedArticle.indexOf(keyword);
  ...
  if (bestIndex < 0) {
    for (const keyword of keywords) {
      const compactArticle = compact(normalizedArticle);
      const compactKeyword = compact(keyword);
      const compactIndex = compactKeyword ? compactArticle.indexOf(compactKeyword) : -1;
      if (compactIndex >= 0) {
        bestIndex = Math.max(0, Math.min(normalizedArticle.length - 1, compactIndex));
        bestKeyword = keyword;
        break;
      }
    }
  ...
  if (bestIndex < 0) {
    return normalizedArticle.slice(0, 1200);
  }

  const start = Math.max(0, bestIndex - 420);
  const end = Math.min(normalizedArticle.length, bestIndex + Math.max(bestKeyword.length, 1) + 900);
  const prefix = start > 0 ? "… " : "";
  const suffix = end < normalizedArticle.length ? " …" : "";

  return `${prefix}${normalizedArticle.slice(start, end)}${suffix}`;
}
...
function renderCandidates(candidates: ArticleCandidate[], confidence: string, rule: Rule) {
  ...
  const body = candidates
    .map((candidate, index) => `[후보 ${index + 1} / 점수 ${candidate.score}]\n관련 부분 발췌:\n${focusedExcerpt(candidate.text, rule)}`)
    .join("\n\n");

  return `자동 추출 신뢰도: ${confidence}\n※ 현행 조항은 관련 후보를 여러 개 표시합니다. PDF 추출 오류가 있을 수 있으므로 후보 조항을 확인하세요.\n\n${body}`;
}
