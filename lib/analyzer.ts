...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    basis: "교육청 권고",
    groups: [["학생선도위원회", "선도위원회"], ["구성", "위원장", "부위원장", "교감", "부장", "위원"], ["과반수", "정족수", "출석", "찬성"]],
    titleKeywords: ["학생선도위원회", "선도위원회", "구성", "정족수"],
    targetKeywords: ["학생선도위원회", "선도위원회", "구성", "위원장", "부위원장", "교감", "부장", "위원", "과반수", "정족수", "개회", "출석", "찬성"],
...
function hasCommitteeQuorumTerms(article: string) {
  // 11번에서 말하는 정족수는 단순한 '징계 의결'이 아니라
  // 과반수, 정족수, 출석+찬성 같은 의사·의결정족수 표현이어야 한다.
...
function isChecklist11CandidateAllowed(article: string) {
  ...
  const hasStructure = hasCommitteeStructureTerms(article);
  const hasQuorum = hasCommitteeQuorumTerms(article);

  const procedureOnlyTerms = [
    "사안설명",
    "의견청취",
    "의견을 진술",
    "출석하여 의견",
  ...
  const hasProcedureOnly = procedureOnlyTerms.some((word) => includesLoose(article, word));
  const hasExcludedCommittee = excludedCommitteeTerms.some((word) => includesLoose(article, word));

  if (!hasIdentity) return false;
  if (hasExcludedCommittee) return false;

  // 11번은 구성/정족수 항목이다.
  // 학생선도위원회라는 말이 있어도 징계 심의, 의견진술, 징계의결, 학교장 결재, 보고만 있으면 제외한다.
  if (!hasStructure && !hasQuorum) return false;
  if (hasProcedureOnly && !hasStructure && !hasQuorum) return false;
...
function findArticleCandidates(text: string, rule: Rule): ArticleCandidate[] {
  ...
  const rankedBeforeSlice = articles
    .map((article) => ({ text: article, score: scoreArticle(article, rule) }))
    .filter((item) => item.score > 0)
    .filter((item) => {
      if (rule.id !== "cl11-committee-composition-quorum") return true;
      return isChecklist11CandidateAllowed(item.text);
    })
    .sort((a, b) => b.score - a.score);

  const ranked = rankedBeforeSlice.slice(0, maxCandidates);

  if (ranked.length > 0) return ranked;

  if (rule.id === "cl11-committee-composition-quorum") {
    return [];
  }
...
function renderCandidates(candidates: ArticleCandidate[], confidence: string, rule: Rule) {
  if (candidates.length === 0) {
    return `자동 추출 신뢰도: 낮음\n관련 조항을 자동으로 찾지 못했습니다. 특히 11번의 경우 학생선도위원회 구성·위원장·부위원장·개회정족수·의결정족수 조항을 담당자가 직접 확인해야 합니다.`;
  }
